  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow group">
      ...
      <Link to={`/item/${item.id}`}>
        <div className="relative h-36 md:h-48 overflow-hidden bg-slate-100">
          <img
          ...
          <span className={`absolute top-2 left-2 text-xs font-semibold px-2 py-0.5 rounded-full ${colorClass}`}>
            {item.category}
          </span>
        </div>
      </Link>
      ...
      <div className="p-3 md:p-4">
        <Link to={`/item/${item.id}`}>
          <h3 className="font-semibold text-slate-800 text-sm md:text-base line-clamp-1 hover:text-teal-600 transition-colors">
            {item.title}
          </h3>
          <p className="text-slate-500 text-xs md:text-sm mt-0.5 line-clamp-2 leading-snug">{item.description}</p>
        </Link>
        ...
        {item.status !== 'gifted' && (
          <a
            ...
            className="mt-2.5 flex items-center justify-center gap-1.5 bg-[#25D366] active:bg-[#20ba58] text-white text-xs md:text-sm font-medium py-2.5 rounded-xl transition-colors"
          >
