...
import { ArrowLeft, MapPin, Clock, CheckCircle2, Loader2, Share2 } from 'lucide-react'
...
export default function ItemDetail() {
  ...
  async function handleMarkGifted() {
    if (!confirm(D.confirmGifted)) return
    setGifting(true)
  ...
  if (!item) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-slate-500">
        <p>{D.notFound}</p>
        <button onClick={() => navigate('/')} className="text-teal-600 underline">{D.backBtn}</button>
      </div>
    )
  }
          ...
          <div className="p-6 md:p-8">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-2 flex-wrap">
                  ...
                  <span className="flex items-center gap-1 text-sm text-slate-400">
                    <Clock size={14} /> {D.listed} {timeAgo}
                  </span>
                </div>
              </div>
            </div>
            ...
            {giftedNow && (
              <div className="mt-4 bg-teal-50 border border-teal-100 rounded-xl p-4 flex items-center gap-3 text-teal-700">
                ...
                <p className="text-sm font-medium">{D.amazingMsg}</p>
              </div>
            )}
          </div>
        </div>
